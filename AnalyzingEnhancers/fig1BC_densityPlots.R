...
enh_regions_width <- lapply(path, function(x){width(anchorOne(readRDS(x)))})
names(enh_regions_width) <- str_replace(str_replace(path,".rds",""),"./","")
...
# get lengths
listTOplot <- lapply(enh_regions_width[c("FOCS","GeneHancer","JEME","EnhancerAtlas")],log10)
plotData <- as.data.frame(unlist(listTOplot))
# get appropriate names
tmp <- sapply(listTOplot, length)
...
# v1
png("/data/akalin/Projects/AAkalin_Catalog_RI/final_Version/Results/Downstream/Plots/figure1_publicationE/SizeDistributionEnhancers_Publications.png"
...
# v1 - legend
png("/data/akalin/Projects/AAkalin_Catalog_RI/final_Version/Results/Downstream/Plots/figure1_publicationE/SizeDistributionEnhancers_Publications_nolegend.png"
    ,width = 900,height = 900)
